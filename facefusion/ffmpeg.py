...
def extract_frames(target_path : str, output_frames_path : str, video_resolution : str, video_fps : Fps) -> bool:
    temp_frame_compression = round(31 - (facefusion.globals.temp_frame_quality * 0.31))
    trim_frame_start = facefusion.globals.trim_frame_start
    trim_frame_end = facefusion.globals.trim_frame_end
    temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d', output_frames_path)
...
def compress_image(output_path : str):
...
def merge_video(frames_path : str, output_path : str, video_resolution : str, video_fps : Fps):
    # temp_output_video_path = get_temp_output_video_path(target_path, output_path)
    frames_pattern = os.path.join(frames_path, '%04d' + '.' + facefusion.globals.temp_frame_format)
    # temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d', frame_path)
    commands = [ '-hwaccel', 'auto', '-s', str(video_resolution), '-r', str(video_fps), '-i', frames_pattern, '-c:v', facefusion.globals.output_video_encoder ]
...
def restore_audio(template_video_path : str, no_audio_path : str, output_path : str, video_fps : Fps) -> bool:
    trim_frame_start = facefusion.globals.trim_frame_start
    trim_frame_end = facefusion.globals.trim_frame_end
    # temp_output_video_path = get_temp_output_video_path(target_path)
    commands = [ '-hwaccel', 'auto', '-i', no_audio_path]
    ...
    commands.extend([ '-i', template_video_path, '-c', 'copy', '-map', '0:v:0', '-map', '1:a:0', '-shortest', '-y', output_path ])
    return run_ffmpeg(commands)
